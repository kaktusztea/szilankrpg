...
def validate():
    ...
    for r in recs:
        ...
        if isinstance(r, dict):
            ...
            # elvart mélyebb ellenőrzés: aktor érvényes + [TÉ, VÉ] két egész
            if isinstance(r.get("elvart"), dict):
                for aktor, val in r["elvart"].items():
                    if aktor not in forras["aktor"]:
                        hibak.append(f"{cimke}.elvart: ismeretlen aktor {aktor!r}")
                    if not (isinstance(val, list) and len(val) == 2
                            and all(isinstance(x, int) and not isinstance(x, bool) for x in val)):
                        hibak.append(f"{cimke}.elvart[{aktor}]: [TÉ, VÉ] két egész kell, kapott {val!r}")
    return hibak
