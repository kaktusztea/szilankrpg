...
export function SlotRow({ slot: s, displayName, active, onLoad, onSaveOptions, onDuplicate, onDelete }: Props) {
  ...
  return (
    <div className={`slot-row ${active ? 'slot-row-active' : ''}`} onClick={() => onLoad(s.uid)}>
      <div className="slot-row-top">
        <span className={`slot-name${activeCls}`}>{displayName}</span>
      ...
      <div className="slot-chips">
        ...
        <button className="slot-chip slot-chip-del" title="Törlés" onClick={e => { e.stopPropagation(); onDelete(s.uid, `${displayName} (${s.tsz || '?'}sz)`); }}>✕</button>
      </div>
    </div>
  );
}
