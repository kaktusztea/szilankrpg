 ...
 */
export function useHoldRepeat(onTick: (dir: 1 | -1) => void, opts?: { initialDelay?: number; minDelay?: number; factor?: number }) {
  const { initialDelay = 200, minDelay = 50, factor = 0.85 } = opts ?? {};
  ...
  const startHold = useCallback((dir: 1 | -1) => {
    ...
    holdRef.current.timer = setTimeout(tick, delay);
  }, [onTick, initialDelay, minDelay, factor]);
