...
export function MisztikusScreen({ data, karakter, képzettségek, setKépzettségek, gameMode }: {
  ...
  const fajMiszt = képzettségek.find(k => k.név.startsWith('Faj misztérium'));
  ...
  const fajMisztDefs = data.kepzettsegDefs.filter(d => d.név.startsWith('Faj misztérium:'));
  ...
  return (
    <div className="screen" style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
        <div style={{ background: 'var(--input-bg)', border: '1px solid #444', borderRadius: '6px', padding: '8px 14px' }}>
          <span style={{ fontSize: '12px', color: '#aaa' }}>Aura</span><br/>
          <strong style={{ fontSize: '20px' }}>{aura}</strong>
        </div>
        <div style={{ background: 'var(--input-bg)', border: '1px solid #444', borderRadius: '6px', padding: '8px 14px' }}>
          <span style={{ fontSize: '12px', color: '#aaa' }}>Mágiaellenállás</span><br/>
          <strong style={{ fontSize: '20px' }}>{me}</strong>
        </div>
        <div style={{ background: 'var(--input-bg)', border: '1px solid #444', borderRadius: '6px', padding: '8px 14px' }}>
          <span style={{ fontSize: '12px', color: '#aaa' }}>Mágia akarata</span><br/>
      ...
      {/* Tradíció */}
      <section>
        <h3 style={{ fontSize: '14px', color: '#aaa', margin: '0 0 6px' }}>Tradíció</h3>
      ...
      {/* Arkánumok — csak ha van tradíció VAGY már van felvett arkánum */}
      {(tradíció || arkánumok.length > 0) && (
        <section>
          <h3 style={{ fontSize: '14px', color: '#aaa', margin: '0 0 6px' }}>Arkánumok</h3>
          {arkánumok.map(a => (
      ...
      {/* Faj misztérium */}
      <section>
        <h3 style={{ fontSize: '14px', color: '#aaa', margin: '0 0 6px' }}>Faj misztérium</h3>
        {fajMiszt && renderRow(fajMiszt)}
        {!fajMiszt && !gameMode && (
          <select className="he-add-select" value="" onChange={e => { if (e.target.value) addKépzettség(e.target.value); }}>
            <option value="">+ Faj misztérium...</option>
            {fajMisztDefs.map(d => <option key={d.név} value={d.név}>{d.név.split(':')[1].trim()}</option>)}
          </select>
        )}
        {!fajMiszt && gameMode && <span style={{ color: '#666', fontSize: '13px' }}>—</span>}
      </section>

      {/* Ősi nyelv ismerete */}
      <section>
        <h3 style={{ fontSize: '14px', color: '#aaa', margin: '0 0 6px' }}>Ősi nyelv ismerete</h3>
