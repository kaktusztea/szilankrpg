...
import { isHelyzetAvailable, getMinPengeWarning, getHelyzetInfoText, buildFegyverOpciók } from './AktivHelpers';
...
export function AktivScreen({ data, karakter, session, setSession, pushUndo }: Props) {
  ...
  useEffect(() => {
    if (!showHelyzetPicker && !showFegyverfogás) return;
    function onKey(e: KeyboardEvent) {
      if (e.key === 'Escape') {
        setShowHelyzetPicker(false); setShowFegyverfogás(false);
      }
    }
    document.addEventListener('keydown', onKey);
    return () => document.removeEventListener('keydown', onKey);
  }, [showHelyzetPicker, showFegyverfogás]);
