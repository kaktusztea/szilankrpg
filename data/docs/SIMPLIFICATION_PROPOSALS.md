...
---
## 4. Hárítófegyver használat — implementációs egyszerűsítés (szabályrendszer változatlan)

**Probléma:** A fortély bináris kapu: van → hárítóVÉ számít, nincs → 0. Ehhez a jelenlegi implementáció az Alapeset rendszert (§16.1) használja: 0.fok override + 1.fok (üres). Ez az Alapeset rendszer legkomplexebb használata a legegyszerűbb logikára.

**Javaslat:** A `calcFogas()` kezelje közvetlenül:

```typescript
if (fegyverfogás === 'fegyver_hárító') {
  const hasFortély = karakter.fortélyok.some(f => f.név === 'Hárítófegyver használat');
  hárítóVÉ = hasFortély ? (hárítóFegyver.VÉ + mfVÉ) : 0;
}
```

**Megszűnik:**
- A fortély yaml 0.fok Alapeset entry (override mód, fegyverfogás feltétel-típus)
- Az `evaluateAlapesetek()` kiértékelése erre a fortélyra

A fortély yaml 1-3 fok marad (ha van más hatásuk — jelenleg nincs). Ha nem ad semmit a fokokon → a fortély yaml-ból is törölhető a módosítók lista, csak a `követelmények` marad (képzettség ellenőrzés).

**Szabályrendszer hatás:** Nincs. A md/ azt mondja: "Hárítófegyver használat fortély nélkül a hárítófegyver VÉ-je nem számít." A webapp ugyanezt teszi — csak nem az Alapeset rendszeren keresztül.

---
## 5. Természetes fegyver — fegyver-szintű override (implementációs egyszerűsítés)
...
---
## 6. Fárasztás — maradjon taktika, de a nem-kalkulált rész legyen tisztán informatív
...
---
## 7. Merevvértviselet Alapeset — törlés (implementációs)
...
| 4   | Hárítófegyver Alapeset       | Implementáció        | ❌ Nem                    |
| 5   | Természetes fegyver override | Implementáció        | ❌ Nem                    |
| 6   | Fárasztás informatív jelölés | Data layer tisztítás | ❌ Nem                    |
| 7   | Merevvértviselet Alapeset    | Implementáció        | ❌ Nem                    |
