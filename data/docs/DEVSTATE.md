...
│   │   │   ├── fortely-mods.ts ← Fortély módosító kiértékelés (önálló modul)
...
│   │   │   ├── utils.ts      ← Közös utility (lookupFegyver, evaluateFeltétel, describeKepChange)
│   │   │   └── index.ts
│   │   ├── hooks/             ← React hook-ok
...
│   │   │   ├── useAutoSave.ts         ← localStorage auto-mentés logika
│   │   │   ├── useGameDataLoader.ts   ← GameData betöltés + karakter inicializálás
│   │   │   ├── useGameModeTabSync.ts  ← Game/Szerkesztő mód váltás tab korrekció
│   │   │   ├── useTaktikaInvalidation.ts ← Taktika fok invalidáció fortély törléskor
│   │   │   ├── useVersionHint.ts      ← Verzió double-tap hint kezelés
│   │   │   └── useHoldRepeat.ts       ← Hold-to-repeat gomb viselkedés (gyorsulás)
│   │   ├── components/
...
│   │   │   ├── PopupOverlay.tsx       ← Generikus popup overlay (createPortal wrapper)
│   │   │   ├── MdLink.tsx             ← Szabályrendszer markdown link (🔗 ikon)
│   │   │   ├── KepzettsegInfoPanel.tsx ← Képzettség Game mód info panel (próba, kiterjesztések)
│   │   │   ├── formatters.tsx         ← fmtCode + fmtHatás shared helperek
│   │   │   ├── harc/                  ← Harc fül (HarcScreen, EpTable, FegyverTable, HarcReszletek, stb.)
│   │   │   ├── aktiv/                 ← Aktív fül (taktikák, helyzetek, manőver, státusz, hatás pool)
│   │   │   ├── tulajdonsagok/         ← Tul/Képz fül (tulajdonság grid, képzettségek, popups/)
│   │   │   ├── fortelyok/             ← Fortélyok fül (csoportok, felvétel wizard, info panel)
│   │   │   ├── harcertekek/           ← Harcértékek fül (HM, fegyverek, páncél, pajzs, hooks/)
│   │   │   ├── tavharc/               ← Távharc fül (CÉ/VÉ kalkulátor, fegyverek, CM)
│   │   │   ├── misztikus/             ← Misztikus fül (Aura, Tradíció, Arkánumok, fortélyok, popups/)
│   │   │   ├── hatterek/              ← Hátterek fül (TagCloud, KarmaCloud, FreeTextPopup)
│   │   │   └── overlays/              ← Overlay komponensek (menü, mentés, slot, undo, stb.)
│   │   └── styles/
│   │       ├── variables.css          ← CSS változók, reset, globális stílusok
│   │       ├── common.css             ← Közös komponens stílusok (gombok, sorok, chipek)
│   │       ├── layout.css             ← App layout (header, tab bar, main)
│   │       ├── overlays.css           ← Overlay/popup stílusok
...
- Generált JSON mezők: `md_fájl` (szabályrendszer link: `fortelyok.{alcsoport}/{fájlnév}.md`)
...
- Fortély módosítók: `engine/fortely-mods.ts` → `calcFortelyMods()` (önálló engine modul)
