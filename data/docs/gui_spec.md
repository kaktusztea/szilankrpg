...
- **Fázisok lépegetése**: M/V/E fázisok egymás után; az aktív fázis diszkrét hideg-kék bal-keret + label. Fázisonként két gomb, középre igazítva. A szín az ALKALMAZÓ szempontját tükrözi: aktívban a manőver-siker felé vezető gomb zöld (nekem jó), a kudarc felé piros; PASSZÍVban fordul — ott ÉN védekezem, így a manőver-siker felé vezető gomb PIROS (nekem rossz), a kudarc felé ZÖLD. A végeredmény-sáv színe ugyanígy mód-függő (passzívban „Manőver sikeres ellened" = piros). A FELIRATOK fázis+mód-specifikusak (`getFázisFelirat`), a konkrét dobást írják le: M — aktív „Elhibázta/Eltalált", passzív „Elhibáztam/Eltaláltam"; V — aktív „Talált/Nem talált", passzív „Eltalált/Nem talált"; E — aktív „Elértem/Nem értem el", passzív „Elérte/Nem érte el". A belső siker-számítás a MANŐVER (támadó) szemszögéből megy — Megakasztásnál a reprezentáció invertált (`handleSiker`). Ha a manővernek van `fázis_info[fázis]` szövege (pl. speciális `M*` — az alkalmazó adja a Megakasztást), az aktív fázis tetején hideg-kék `ⓘ` magyarázó sor jelenik meg. `M`/`V` fázisnál a magyarázat HELYETTESÍTI a fix érték-kijelzést (elkerüli az ellentmondást, pl. Távoltartás: nem „Ellenfél támad a VÉ-d ellen"); `E` fázisnál KIEGÉSZÍTŐ (a dobás-UI — módosítók, MP, célszám — mindig megmarad).
